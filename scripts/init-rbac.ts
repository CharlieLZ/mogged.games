...
import { buildUserRoleMatchCondition } from '@/shared/services/rbac';
...
async function initializeRBAC() {
  ...
  if (adminEmailArg) {
    ...
    if (adminUser) {
      ...
      const [existingUserRole] = await db()
        .select()
        .from(userRole)
        .where(buildUserRoleMatchCondition(adminUser.id, superAdminRoleId));
  ...
  console.log('\n💡 Next steps:');
  console.log('   1. Sync database schema with `pnpm db:push` if needed');
