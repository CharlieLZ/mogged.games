...
export class CreemProvider implements PaymentProvider {
    ...
    try {
      ...
      if (eventType === PaymentEventType.CHECKOUT_SUCCESS) {
        ...
        paymentSession = {
          ...
          paymentResult: obj,
          metadata: obj?.metadata || event.metadata,
          subscriptionId:
            obj?.subscription?.id ||
            obj?.subscription_id ||
            obj?.subscriptionId,
        ...
        if (obj?.subscription) {
          paymentSession.subscriptionInfo = await this.buildSubscriptionInfo(
            obj.subscription,
            obj.subscription.product,
            rawEventType
          );
          paymentSession.subscriptionResult = obj.subscription;
        }
      }
  ...
  private async buildSubscriptionInfo(
    ...
    const eventStatus = rawEventType?.startsWith('subscription.')
      ? rawEventType.replace('subscription.', '')
      : undefined;
    const normalizedStatus = rawStatus || eventStatus;

    const isScheduledCancel =
      normalizedStatus === 'scheduled_cancel' ||
      eventStatus === 'scheduled_cancel';
    const isExpired =
      normalizedStatus === 'expired' || eventStatus === 'expired';
    const isUnpaid =
      normalizedStatus === 'unpaid' || eventStatus === 'unpaid';
    const isPastDue =
      normalizedStatus === 'past_due' || eventStatus === 'past_due';

    const { interval, count: intervalCount } = this.mapCreemInterval(product);

    const cancelAtCandidate =
    ...
    const subscriptionInfo: SubscriptionInfo = {
      ...
      metadata: {
        ...
        creem_status: normalizedStatus,
      },
    };

    if (normalizedStatus === 'active') {
      if (subscription.cancel_at || isScheduledCancel) {
        subscriptionInfo.status = SubscriptionStatus.PENDING_CANCEL;
        if (cancelAtCandidate) {
          subscriptionInfo.canceledAt = new Date(cancelAtCandidate);
          subscriptionInfo.canceledEndAt = new Date(cancelAtCandidate);
        }
      } else {
      ...
      subscriptionInfo.status = SubscriptionStatus.CANCELED;
      subscriptionInfo.canceledReasonType = 'expired';
      if (cancelAtCandidate) {
        subscriptionInfo.canceledAt = new Date(cancelAtCandidate);
      }
    ...
    } else if (normalizedStatus === 'incomplete' || normalizedStatus === 'incomplete_expired') {
      // 首次支付未完成或已过期，跳过处理
      subscriptionInfo.status = SubscriptionStatus.PAUSED;
      subscriptionInfo.metadata = {
        ...(subscriptionInfo.metadata || {}),
      ...
      throw new Error(
        `Unknown Creem subscription status: ${normalizedStatus}`
      );
    }
  ...
  private mapCreemInterval(product: any): {
    ...
    // 如果 product 或 billing_period 不存在，返回默认值（月付）
    if (!product || !product.billing_period) {
      console.warn('[Creem] Product or billing_period missing, using default interval');
      return {
