  ...
  const form: Form = {
    fields: [
      ...
      },
      {
        ...
        value: getDayjs(subscription.createdAt).format('YYYY-MM-DD'),
        attributes: { disabled: true },
      },
      {
        name: 'currentPeriod',
        title: t('fields.current_period'),
        value: `${getDayjs(subscription.currentPeriodStart).format('YYYY-MM-DD')} ~ ${getDayjs(subscription.currentPeriodEnd).format('YYYY-MM-DD')}`,
        attributes: { disabled: true },
      },
    ],
    data: subscription,
