...
export async function getUserRolesBatch(
  ...
  // Initialize all userIds with empty array
  userIds.forEach((id) => rolesMap.set(id, []));
  // Group results by userId
  result.forEach((r) => {
    const { userId, ...roleData } = r;
    const userRoles = rolesMap.get(userId) || [];
    userRoles.push(roleData as Role);
    rolesMap.set(userId, userRoles);
  });
...
export function buildUserRoleMatchCondition(userId: string, roleId: string) {
...
export async function assignRoleToUser(
  ...
  const [existing] = await db()
    .select()
    .from(userRole)
    .where(buildUserRoleMatchCondition(userId, roleId));
...
export async function removeRoleFromUser(
  ...
  await db().delete(userRole).where(buildUserRoleMatchCondition(userId, roleId));
}
