import { and, count, desc, eq, isNull } from 'drizzle-orm';

import { db } from '@/core/db';
import { credit, order, subscription, user } from '@/config/db/schema';
import { PaymentType } from '@/extensions/payment/types';

import { NewCredit } from './credit';
import {
  NewSubscription,
  UpdateSubscription,
  updateSubscriptionBySubscriptionNo,
} from './subscription';
import { appendUserToResult, User } from './user';

export type Order = typeof order.$inferSelect & {
  user?: User;
};
export type NewOrder = typeof order.$inferInsert;
export type UpdateOrder = Partial<
  Omit<NewOrder, 'id' | 'orderNo' | 'createdAt'>
>;

export enum OrderStatus {
  // processing status
  PENDING = 'pending', // order saved, waiting for checkout
  CREATED = 'created', // checkout success
  // final status
  COMPLETED = 'completed', // checkout completed, but failed
  PAID = 'paid', // order paid success
  FAILED = 'failed', // order paid, but failed
}

/**
 * create order
 */
export async function createOrder(newOrder: NewOrder) {
  const [result] = await db().insert(order).values(newOrder).returning();

  return result;
}

/**
 * get orders
 */
export async function getOrders({
  orderNo,
  userId,
  userEmail,
  status,
  getUser,
  paymentType,
  paymentProvider,
  page = 1,
  limit = 30,
}: {
  orderNo?: string;
  userId?: string;
  userEmail?: string;
  status?: OrderStatus;
  getUser?: boolean;
  paymentType?: PaymentType;
  paymentProvider?: string;
  page?: number;
  limit?: number;
} = {}): Promise<Order[]> {
  // If userEmail is provided, find the userId first
  let effectiveUserId = userId;
  if (userEmail && !userId) {
    const [foundUser] = await db()
      .select({ id: user.id })
      .from(user)
      .where(eq(user.email, userEmail));
    if (foundUser) {
      effectiveUserId = foundUser.id;
    } else {
      // No user found with this email, return empty array
      return [];
    }
  }

  const result = await db()
    .select()
    .from(order)
    .where(
      and(
        orderNo ? eq(order.orderNo, orderNo) : undefined,
        effectiveUserId ? eq(order.userId, effectiveUserId) : undefined,
        status ? eq(order.status, status) : undefined,
        paymentType ? eq(order.paymentType, paymentType) : undefined,
        paymentProvider
          ? eq(order.paymentProvider, paymentProvider)
          : undefined,
        isNull(order.deletedAt)
      )
    )
    .orderBy(desc(order.createdAt))
    .limit(limit)
    .offset((page - 1) * limit);

  if (getUser) {
    return appendUserToResult(result);
  }

  return result;
}

/**
 * get orders count
 */
export async function getOrdersCount({
  orderNo,
  userId,
  userEmail,
  paymentType,
  status,
  paymentProvider,
}: {
  orderNo?: string;
  userId?: string;
  userEmail?: string;
  paymentType?: PaymentType;
  paymentProvider?: string;
  status?: OrderStatus;
} = {}): Promise<number> {
  // If userEmail is provided, find the userId first
  let effectiveUserId = userId;
  if (userEmail && !userId) {
    const [foundUser] = await db()
      .select({ id: user.id })
      .from(user)
      .where(eq(user.email, userEmail));
    if (foundUser) {
      effectiveUserId = foundUser.id;
    } else {
      // No user found with this email, return 0
      return 0;
    }
  }

  const [result] = await db()
    .select({ count: count() })
    .from(order)
    .where(
      and(
        orderNo ? eq(order.orderNo, orderNo) : undefined,
        effectiveUserId ? eq(order.userId, effectiveUserId) : undefined,
        status ? eq(order.status, status) : undefined,
        paymentType ? eq(order.paymentType, paymentType) : undefined,
        paymentProvider
          ? eq(order.paymentProvider, paymentProvider)
          : undefined,
        isNull(order.deletedAt)
      )
    );

  return result?.count || 0;
}

/**
 * find order by id
 */
export async function findOrderById(id: string) {
  const [result] = await db()
    .select()
    .from(order)
    .where(and(eq(order.id, id), isNull(order.deletedAt)));

  return result;
}

/**
 * find order by order no
 */
export async function findOrderByOrderNo(orderNo: string) {
  const [result] = await db()
    .select()
    .from(order)
    .where(and(eq(order.orderNo, orderNo), isNull(order.deletedAt)));

  return result;
}

export async function findOrderByProviderTransactionId({
  provider,
  transactionId,
}: {
  provider: string;
  transactionId: string;
}) {
  const [result] = await db()
    .select()
    .from(order)
    .where(
      and(
        eq(order.paymentProvider, provider),
        eq(order.transactionId, transactionId),
        isNull(order.deletedAt)
      )
    )
    .limit(1);

  return result;
}

/**
 * update order
 */
export async function updateOrderByOrderNo(
  orderNo: string,
  updateOrder: UpdateOrder
) {
  const [result] = await db()
    .update(order)
    .set(updateOrder)
    .where(eq(order.orderNo, orderNo))
    .returning();

  return result;
}

/**
 * update order by order id
 */
export async function updateOrderByOrderId(
  orderId: string,
  updateOrder: UpdateOrder
) {
  const [result] = await db()
    .update(order)
    .set(updateOrder)
    .where(eq(order.id, orderId))
    .returning();

  return result;
}

export async function updateOrderInTransaction({
  orderNo,
  updateOrder,
  newSubscription,
  newCredit,
}: {
  orderNo: string;
  updateOrder: UpdateOrder;
  newSubscription?: NewSubscription;
  newCredit?: NewCredit;
}) {
  if (!orderNo || !updateOrder) {
    throw new Error('orderNo and updateOrder are required');
  }

  // only update order, no need transaction
  if (!newSubscription && !newCredit) {
    return updateOrderByOrderNo(orderNo, updateOrder);
  }

  // need transaction
  const result = await db().transaction(async (tx) => {
    const result: any = {
      order: null,
      subscription: null,
      credit: null,
      creditCreated: false,
    };

    // deal with subscription
    if (newSubscription) {
      let existingSubscription: any = null;
      if (newSubscription.subscriptionId && newSubscription.paymentProvider) {
        // not create subscription with same subscription id and payment provider
        const [existingSubscriptionResult] = await tx
          .select()
          .from(subscription)
          .where(
            and(
              eq(subscription.subscriptionId, newSubscription.subscriptionId),
              eq(subscription.paymentProvider, newSubscription.paymentProvider),
              isNull(subscription.deletedAt)
            )
          );

        existingSubscription = existingSubscriptionResult;
      }

      if (!existingSubscription) {
        // create subscription
        const [subscriptionResult] = await tx
          .insert(subscription)
          .values(newSubscription)
          .onConflictDoNothing()
          .returning();

        existingSubscription = subscriptionResult;
      }

      if (
        !existingSubscription &&
        newSubscription.subscriptionId &&
        newSubscription.paymentProvider
      ) {
        const [existingSubscriptionResult] = await tx
          .select()
          .from(subscription)
          .where(
            and(
              eq(subscription.subscriptionId, newSubscription.subscriptionId),
              eq(subscription.paymentProvider, newSubscription.paymentProvider),
              isNull(subscription.deletedAt)
            )
          );

        existingSubscription = existingSubscriptionResult;
      }

      result.subscription = existingSubscription;
    }

    // deal with credit
    if (newCredit) {
      // not create credit with same order no
      let [existingCredit] = await tx
        .select()
        .from(credit)
        .where(and(eq(credit.orderNo, orderNo), isNull(credit.deletedAt)));

      if (!existingCredit) {
        // create credit
        const [creditResult] = await tx
          .insert(credit)
          .values(newCredit)
          .onConflictDoNothing()
          .returning();

        existingCredit = creditResult;
        result.creditCreated = true;
      }

      if (!existingCredit) {
        const [existingCreditResult] = await tx
          .select()
          .from(credit)
          .where(and(eq(credit.orderNo, orderNo), isNull(credit.deletedAt)));

        existingCredit = existingCreditResult;
        result.creditCreated = false;
      }

      result.credit = existingCredit;
    }

    // update order
    const [orderResult] = await tx
      .update(order)
      .set(updateOrder)
      .where(eq(order.orderNo, orderNo))
      .returning();

    result.order = orderResult;

    return result;
  });

  return result;
}

export async function updateSubscriptionInTransaction({
  subscriptionNo,
  updateSubscription,
  newOrder,
  newCredit,
}: {
  subscriptionNo: string; // subscription unique id in table
  updateSubscription: UpdateSubscription;
  newOrder?: NewOrder;
  newCredit?: NewCredit;
}) {
  if (!subscriptionNo || !updateSubscription) {
    throw new Error('subscriptionNo and updateSubscription are required');
  }

  // only update order, no need transaction
  if (!newOrder && !newCredit) {
    return updateSubscriptionBySubscriptionNo(
      subscriptionNo,
      updateSubscription
    );
  }

  // need transaction
  const result = await db().transaction(async (tx) => {
    const result: any = {
      order: null,
      subscription: null,
      credit: null,
      creditCreated: false,
    };

    // deal with order
    if (newOrder) {
      let existingOrder: any = null;
      if (newOrder.transactionId && newOrder.paymentProvider) {
        // not create order with same payment transaction id and payment provider
        const [existingOrderResult] = await tx
          .select()
          .from(order)
          .where(
            and(
              eq(order.transactionId, newOrder.transactionId),
              eq(order.paymentProvider, newOrder.paymentProvider),
              isNull(order.deletedAt)
            )
          );

        existingOrder = existingOrderResult;
      }

      if (!existingOrder) {
        // create order
        const [orderResult] = await tx
          .insert(order)
          .values(newOrder)
          .onConflictDoNothing()
          .returning();

        existingOrder = orderResult;
      }

      if (
        !existingOrder &&
        newOrder.transactionId &&
        newOrder.paymentProvider
      ) {
        const [existingOrderResult] = await tx
          .select()
          .from(order)
          .where(
            and(
              eq(order.transactionId, newOrder.transactionId),
              eq(order.paymentProvider, newOrder.paymentProvider),
              isNull(order.deletedAt)
            )
          );

        existingOrder = existingOrderResult;
      }

      result.order = existingOrder;
    }

    // deal with credit
    if (newCredit) {
      let existingCredit: any = null;
      if (result.order && result.order.orderNo) {
        // not create credit with same order no
        const [existingCreditResult] = await tx
          .select()
          .from(credit)
          .where(
            and(
              eq(credit.orderNo, result.order.orderNo),
              isNull(credit.deletedAt)
            )
          );

        existingCredit = existingCreditResult;
      }

      if (!existingCredit) {
        const [creditResult] = await tx
          .insert(credit)
          .values(newCredit)
          .onConflictDoNothing()
          .returning();

        existingCredit = creditResult;
        result.creditCreated = true;
      }

      if (!existingCredit && result.order?.orderNo) {
        const [existingCreditResult] = await tx
          .select()
          .from(credit)
          .where(
            and(
              eq(credit.orderNo, result.order.orderNo),
              isNull(credit.deletedAt)
            )
          );

        existingCredit = existingCreditResult;
        result.creditCreated = false;
      }

      result.credit = existingCredit;
    }

    // update subscription
    const [subscriptionResult] = await tx
      .update(subscription)
      .set(updateSubscription)
      .where(eq(subscription.subscriptionNo, subscriptionNo))
      .returning();

    result.subscription = subscriptionResult;

    return result;
  });

  return result;
}
