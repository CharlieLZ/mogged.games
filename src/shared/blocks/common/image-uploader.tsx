...
const uploadImageFile = async (file: File) => {
  const result = await uploadFileWithDirectStorage(file, {
    ...
    Promise.all(
      newItems.map(async (item) => {
        try {
          ...
          console.error('[image-uploader/upload_failed]', error);
          toast.error(
            error?.message ? `${t('failed')}: ${error.message}` : t('failed')
          );
          setItems((prev) => {
            const next = prev.map((current) =>
              current.id === item.id
        ...
        {items.map((item) => (
          ...
          >
            <div className="relative overflow-hidden rounded-lg">
              ...
              />
              {item.size && (
                <span className="bg-background text-muted-foreground absolute bottom-2 left-2 rounded-md px-2 py-1 text-xs font-medium rtl:right-2 rtl:left-auto">
                  {formatBytes(item.size)}
                </span>
              )}
              {item.status === 'uploading' && (
                <div className="bg-foreground/60 text-background absolute inset-0 flex items-center justify-center text-xs font-medium">
                  {t('uploading')}
                </div>
              )}
              {item.status === 'error' && (
                <div className="bg-destructive/75 text-destructive-foreground absolute inset-0 flex items-center justify-center text-xs font-medium">
                  {t('failed')}
                </div>
              )}
              <Button
                size="icon"
                variant="destructive"
                className="absolute top-2 right-2 h-7 w-7 opacity-0 transition-opacity group-hover:opacity-100 rtl:right-auto rtl:left-2"
